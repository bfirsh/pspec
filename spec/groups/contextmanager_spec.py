...
with describe('ContextManagerGroup'):
    ...
    def it_creates_a_spec_on_the_module():
        assert hasattr(single_level_spec, '_pspec')

    def it_creates_a_group_with_a_subject():
        assert isinstance(single_level_spec._pspec.children[0], ContextManagerGroup)
        assert single_level_spec._pspec.children[0].subject == 'random'

    def it_creates_tests_from_functions_within_context_manager():
        spec = single_level_spec._pspec
    ...
    def it_creates_children_with_a_subject():
        parent = multiple_level_spec._pspec.children[0]
    ...
    def it_creates_children_in_correct_order():
        group = random_spec._pspec.children[0]
